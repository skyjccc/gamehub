...
| 游戏 | 服务 | 入口 |
|---|---|---|
| 拍档大冒险 | `node WWW/patener/server.js`（依赖 ws） | 端口 8642 |
| 双影同行 | `node WWW/shadow-duo/server.js`（零依赖） | 端口 3000 |
| 内网棋牌室 | `node WWW/qipai/server/index.js`（依赖 ws、chess.js） | 端口 3456 |

这三款是独立 Node + WebSocket 服务，入口地址用 **`{host}` 占位符**（如 `http://{host}:8642/`）：
启动时自动替换成当前访问者使用的主机名 —— 本机打开是 `127.0.0.1:8642`，
手机用局域网 IP 打开大厅就是 `192.168.1.10:8642`，一台配置两端通用。
（游戏库本身也要用局域网可访问的地址打开，如 `http://192.168.1.10:8890/gamehub/`；首次入站连接 Windows 防火墙弹窗请选"允许"。）
